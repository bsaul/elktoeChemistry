...
* clustered within each specimen
* used one transect per specimen: the one with the most of annuli A

In order to test for differential accrual of each element within the nacre, we conducted a series of hypothesis tests described in Table XXXX. In each case, we tested the sharp null hypothesis that each specimen would have developed the same elemental trajectory in terms of $\text{log}(\text{mol/Ca mmol})$ regardless of the experimental site to which it was assigned. Since the allocation to site was completely randomized, inference was drawn using 1000 draws from the randomization distribution, using the `ri2` R package [@coppock2019ri2; @rcore2019]. The method can be summarized as follows: (1) compute the observed test statistic ($t_{obs}$) using the observed allocation, (2) permute the allocation by assigning specimens to different sites and re-compute the test statistic, (3) repeat (2) $m = 1000$ times. The two-sided p-value is the proportion of times $t_{obs}$ is greater than or equal to the test statistic distribution.
...
Morbidity and mortality in *A. raveneliana* varied significantly across sites (Figure \@ref(fig:mortality)). In the Little Tennessee, all but three specimens lost weight or died, whereas in Tuckaseegee Site 1 no individuals died and few lost weight. In both rivers, outcomes worsened downstream for *A. raveneliana*. Mortality increased monotonically down the river gradient, and the median relative weight loss also tended to increase. In constrast to *A. raveneliana*, none of the *L. fasciola* died and all gained weight.
...
In *L. fasciola*, Strontium and Barium have the most evidence for differences. However, small p-value in Strontium appears to be driven by a difference between the baseline specimens and the specimens in experimental sites. This is in constrast to Barium, where the difference appears to be driven by difference among the experimental sites.
