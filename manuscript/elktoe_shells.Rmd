---
title: Methodological Considerations in the Analysis of Trace Element Concentrations 
author:
...
abstract: >
  \noindent Trace element concentrations obtained from Laser Ablation Mass Spectrometry are increasingly used in experiments to compare physiological outcomes. A goal is to determine whether the experiment resulted in differences trace element concentrations between experimental groups. In this paper, we discuss methodological considerations in designing and analyzing these experiments. Multiple factors make analyzing and interpreting results challenging. These include misalignments in chemical and anatomic features, noisy and skewed data where extreme values may be important, censoring in the physiological process due to mortality, and lack in understanding of physiological processes by which to explain observed differences. These lessons are drawn from a completely randomized experiment designed to assess factors associated with physiological outcomes in the federally endangered freshwater mussel Appalachian Elktoe (*Alasmidonta raveneliana*). We present the results of this study, describe the challenges in interpretability, and provide suggestions for future research.
keywords: TODO
---
...
Trace element concentrations obtained from Laser Ablation Mass Spectrometry (LA-ICP-MS) are increasingly used in experiments to study biological processes [@pozebon2014review]. Studies often compare elemental concentrations between groups in order to learn about physiological responses to an experiment or *in situ* environmental factors. Natural variation between individuals may be poorly covered by the study's sample. Thus, in order characterize as much variation in concentrations as possible, each specimen would ideally have both pre- and post-experiment LA-ICP-MS samples. Wince LA-ICP-MS is destructive and anatomical regions of interest are internal, accurately identifying sources of variation within an individual may be impossible in most organisms, even when an experimenter manipulates an instrument designed to elicit a biological response.
...
Valves were grouped into seven batches for LA-ICP-MS processing. During each batch process, the lower limit of detection (LOD) was assessed multiple times using the standards noted above. Concentration values were left-censored based on the mean LOD within a batch for each element. For analysis, parts-per-million (ppm) values were rescaled to $\text{mol/Ca mmol}$ where $\text{Ca mmol}$ was based on Ca\% of ~39.54\% for nacral observations and 40.078\% for all other layers. Chemical signals were denoised by variety of methods including no filtering, a generalized additive model, and a rolling median absolute deviation filter with a bandwidth of ten observations.
